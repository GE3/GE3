...
If( $_POST["zmenitPoradi"] ){
    ...
    Foreach($_POST["vaha"] as $key=>$value){
            If( $key ){
                $value = $value? $value: '0';
                $kategorieAktivni = $_POST["kategorieAktivni"][$key]? 'ano': 'ne';
                If( Mysql_query("UPDATE $CONF[sqlPrefix]zbozi x, $CONF[sqlPrefix]zbozi y SET x.vaha=$value, x.kategorieAktivni='$kategorieAktivni' WHERE x.kategorie=y.kategorie AND y.id=$key") ){
                    $i++;
                }
            }
    }
    Mysql_query("UPDATE $CONF[sqlPrefix]zbozi SET podkat1Aktivni='ne'");
    Foreach($_POST["podkat1Aktivni"] as $key=>$value){
            If( $key ){
                $value = $value? 'ano': 'ne';
                Mysql_query("UPDATE $CONF[sqlPrefix]zbozi x, $CONF[sqlPrefix]zbozi y SET x.podkat1Aktivni='$value' WHERE x.kategorie=y.kategorie AND x.podkat1=y.podkat1 AND y.id=$key");
            }
    }    
    Echo '<div style="border: 2px solid #008000; background-color: #EEFFEE; text-align: center; margin: 4px 6px 4px 6px; padding: 2px 4px 2px 4px; ">
...
While($radekKategorie = mysql_fetch_array($dotazKategorie)){
      ...
      Echo '<div style="margin-top: 4px;">
              <span '.$onClick.' '.$style.'>
                ...
                <input type="checkbox" name="kategorieAktivni['.$radekKategorie["id"].']" value="1" '.( $radekKategorie["kategorieAktivni"]=='ano'? 'checked': '' ).'>
      ...
      If( zjisti_z("$CONF[sqlPrefix]zbozi", "podkat1", "kategorie='".$radekKategorie["kategorie"]."' ORDER BY podkat1 DESC") ){
          ...
          While($radekPodkat1 = Mysql_fetch_array($dotazPodkat1)){
                ...
                Echo '<span '.$onClick.' '.$style.'>
                        '.$obrazek.' 
                        <input type="checkbox" name="podkat1Aktivni['.$radekPodkat1["id"].']" value="1" '.( $radekPodkat1["podkat1Aktivni"]=='ano'? 'checked': '' ).'>
