...
If( $_GET["a"]=='produkty' AND !$_GET["produkt"] ){
    ...
    elseif( $_GET["kategorie"] ){
        $tmplProdukty->newBlok("vypis_kategorii");
        $dotaz = mysql_query("SELECT x.* FROM (SELECT * FROM $CONF[sqlPrefix]zbozi ORDER BY obrazekPodkat1 DESC) x, $CONF[sqlPrefix]zbozi y WHERE x.kategorie=y.kategorie AND y.id='".preg_replace("|^([0-9]*)-.*$|","$1",$_GET["kategorie"])."' AND x.podkat1!='' AND x.podkat1Aktivni='ano' GROUP BY x.podkat1") or die(mysql_error());
        $i=0;
        while($podkat=mysql_fetch_assoc($dotaz)){
              $i++;
    ...
    //akční nabídka
    $podminka.= $_GET["akcni-nabidka"]? "AND akce='1' ": '';
    ...
    while($radek=mysql_fetch_array($dotaz)){
          ...
          // Popis
          $tmplProdukty->prirad("produkt.popis", nl2br(strip_tags($radek["popis"])));
          ...
          if( zjisti_z("$CONF[sqlPrefix]nastaveni","zbozi_puvodni_cena_active","id=1") ){
              ...
              $tmplProdukty->prirad("produkt.puvodni_cena.hodnota_s_dph", $puvodni_cena_s_dph);
              $tmplProdukty->prirad("produkt.puvodni_cena.hodnota_bez_dph", $puvodni_cena_bez_dph);
              ...
              $tmplProdukty->prirad("produkt.puvodni_cena.usetrite", zjisti_z("$CONF[sqlPrefix]nastaveni","zbozi_usetrite_jednotky","id=1")=='Kč'? $usetrite_kc: $usetrite_procent);
              $tmplProdukty->prirad("produkt.puvodni_cena.jednotky", zjisti_z("$CONF[sqlPrefix]nastaveni","zbozi_usetrite_jednotky","id=1"));
          }          
          // Dostupnost
          ...
          If( $obrazek) $tmplProdukty->prirad("produkt.obrazek", "$CONF[absDir]zbozi/obrazky/$obrazek");
